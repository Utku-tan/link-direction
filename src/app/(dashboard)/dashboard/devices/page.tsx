...
import {
  ...
  ShieldAlert, Tag, Coffee, Save, X, Copy, Check
} from 'lucide-react'
...
function DeviceCard({
  ...
  const claimUrl = `https://refly.world/claim/${device.device_serial}`
  ...
  return (
    <div className="rounded-2xl border border-zinc-800 bg-[#18181b]/50 p-5 hover:border-[#00f2fe]/20 transition-all duration-300 space-y-4">
      ...
      <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#09090b]/50 text-xs">
        ...
        <span className="text-zinc-400 truncate flex-1">{shortUrl}</span>
        <button
          onClick={handleCopyClaimUrl}
          className="p-1 rounded-md text-zinc-500 hover:text-[#00f2fe] hover:bg-zinc-800 transition-all shrink-0"
          title="Claim linkini kopyala"
        >
